...
Context.prototype.extends = function (name, callback) {
      ...
      var _layoutFileHandler = function (name, callback) {
        console.log('err... whazzup?')
        var extname = path.extname(name) ? '' : options.ext;
        var filename = path.resolve(process.cwd(), name + extname);
      ...
      console.log('no?')
    _layoutFileHandler(path.join(path.dirname(this._locals.filename),name), callback);
